class SecretsController < ApplicationController
  ...
  # POST /secrets.json
  def create
    theSecret = secret_params
    theSecret[:user_id] = session[:user_id]
    @secret = Secret.new(theSecret)
  ...
  def likes
    @newLike = Like.create(user_id:session[:id],secret_id:params[:id])
    redirect_to secrets
  end
  def unlikes
    @newLike = Like.where(user_id:session[:id],secret_id:params[:id]).destroy
    redirect_to secrets
  end
